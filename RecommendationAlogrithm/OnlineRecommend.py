...
def updateonline(userid,nowmovieid):
    ...
    preVal1 = cosSim.loc[mix_list,recentmovie[:,1]].values.\
                  dot(recentmovie[:,2])/np.sum(cosSim.loc[mix_list,recentmovie[:,1]].values,axis=1)
    highratemovie = recentmovie[:,2]>3  # 用户打分大于3的

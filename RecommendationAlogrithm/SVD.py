...
with open(GlobalVar.pathcosSim_svd,"wb") as file:
...
#shape求每个维度的数目，对于矩阵，shape[0]为行数
...
for user in range(rating_matrix.shape[0]):
    ...
    haverated = ~unrate
    predict_Val=(cosSim[unrate,:][:,haverated]).dot(rating_matrix.values[user,:][haverated])/\
                np.sum(cosSim[unrate,:][:,haverated],axis=1)

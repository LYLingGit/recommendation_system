...
rating_dict = {}
# 遍历rating每行,将数据转换成嵌套字典的形式:{userid:{movieid:rating}}
...
#进行模型的评价
rating_train = pickle.load(open("test_data.pkl",'rb'))
val = pickle.load(open("val.pkl",'rb'))
test_loc = pickle.load(open("test_loc.pkl","rb"))
    ...
    for _ in range(max_iter):
        ...
        if _%10 == 0:
            ...
            if max_r2<r2:
                ...
                with open('P_k12_best.pickle', 'wb') as file:
                    pickle.dump(P, file)
                with open('Q_k12_best.pickle', 'wb') as file:
                    pickle.dump(Q, file)
        if cost<0.001:
            break
    return P,Q,cost
...
#离线推荐系统
#预测用户未评分的电影评分
#对于每个用户取预测评分最高的100部电影，存入predict_matrix中
